...
std::size_t GroceryList::groceries_sl_list_size() const
{
  ///////////////////////// TO-DO //////////////////////////////
    ...
    {
      std::size_t size = 0;
      for (auto current = _groceries_sl_list.cbegin(); current != _groceries_sl_list.cend(); ++ size, ++current);
      return size;
    }
...
GroceryList & GroceryList::operator+=( const std::initializer_list<GroceryItem> & rhs )
{
  ///////////////////////// TO-DO //////////////////////////////
    ...
    {
      GroceryList::insert(item, Position::BOTTOM);
    }
...
GroceryList & GroceryList::operator+=( const GroceryList & rhs )
{
  ...
  Position bottom = Position::BOTTOM;
   ...
   {
     GroceryList::insert(item, bottom);
   }
  ...
  if constexpr ((true))
    {
      ...
      auto it = std::find( _groceries_vector.cbegin(), _groceries_vector.cend(), item );
      return it - _groceries_vector.cbegin();
    }
    else                                   
    {
...
void GroceryList::insert( const GroceryItem & item, std::size_t offsetFromTop )       // insert new item at offsetFromTop, which places it before the current item at offsetFromTop
{
  ...
  /**********  Insert into array  ***********************/
  {
    ///////////////////////// TO-DO //////////////////////////////
      ...
      if ( _groceries_array_size >= _groceries_array.size())  throw GroceryList::CapacityExceeded_ex( "Conainter length error" exception_location );
...
void GroceryList::moveToTop( const GroceryItem & item )
{
  ///////////////////////// TO-DO //////////////////////////////
    ...
    {  
        GroceryList::remove ( offset         );
        GroceryList::insert ( item, position );
