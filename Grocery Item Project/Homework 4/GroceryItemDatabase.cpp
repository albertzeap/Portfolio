  ...
  try
  {
    ...
    ///////////////////////// TO-DO //////////////////////////////
     ...
     std::string upc,
      ...
      while( !fin.eof() )
      { 
             fin  >> std::quoted(  upc       )          >> ignore (','  )
                  >> std::quoted(  brand     )          >> ignore (','  )
                  >> std::quoted(  product   )          >> ignore (','  )   
                  >> price                              >> ignore ( '\n');

        data.insert( std::pair < std::string ,GroceryItem > (upc , GroceryItem( product , brand , upc , price) ));
...
GroceryItem * GroceryItemDatabase::find( const std::string & upc )
{
  ///////////////////////// TO-DO //////////////////////////////
    ...
    for (auto &item : data )
    {
      if (item.first == upc) return  & item.second;
    }
