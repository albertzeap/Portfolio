...
bool operator< (const GroceryItem & lhs, const GroceryItem & rhs)
{
	if 		( lhs._upc         != rhs._upc        ) 		return lhs._upc < rhs._upc;
	else if ( lhs._productName != rhs._productName) 		return lhs._productName < rhs._productName;
	else if ( lhs._brandName   != rhs._brandName  )			return lhs._brandName < rhs._brandName;
	else if ( std::abs(lhs._price - rhs._price) >= EPSILON) return lhs._price < rhs._price;
